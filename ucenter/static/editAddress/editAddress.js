...
function editAddress(){
    this.defaults={
        ...
        saveBtn:".btn",
        inputname:["name","phone","card","detailed","postal"],
        errorstr:["请填写姓名","请填写正确的手机号码","身份证不合法","请填写详细地址","请填写邮政编码"],
        ajaxloaded:true
    };
    this.Verification={
       name:false,
...
editAddress.prototype={
    init:function(){
        ...
        if($(".edit-address").attr("data-type")=="edit"){
          ...
          $.post("http://m.kaluli.com/api/getAddress",{id:id},function(data){
              ...
              $.get("http://m.kaluli.com/api/getNextRegionById?id="+addressdata.province,function(data){                                                                                  
    ...
    bindFun:function(){
        ...
        $("textarea[name='detailed']").bind('blur',function(){
            var val = $(this).val();
            if($.trim(val) == ""){                                                 
                  $.tips(errorstr[3]);
        ...
        $(that.defaults.saveBtn).on("tap",function(){                                       
            ...
            var $this = $(this),
                ...
                cityVal = $(".sel-2").val(),
                areaVal = $(".sel-3").val(),
                streetVal = $("textarea[name='detailed']").val(),
